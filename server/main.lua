...
function getIdentity(source)
	...
	for k,v in ipairs(players[ide]) do
		return {
			firstname = v.imie,
			lastname = v.nazwisko
		}
...
RegisterServerEvent('akamis:startujtabelezimi')
AddEventHandler('akamis:startujtabelezimi', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local ide = xPlayer.identifier
	if players[ide] == nil then 
		players[ide] = {}
	end
	MySQL.Async.fetchAll('SELECT firstname, lastname FROM users WHERE identifier = @identifier', {
		['@identifier'] = ide
	}, function(result)
		table.insert(players[ide], {imie = result[1].firstname, nazwisko = result[1].lastname})
...
RegisterCommand('lspd', function(source, args, rawCommand)
	...
	if args[1] ~= nil then
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx:showNotification', source, '~r~Nie pracujesz w LSPD!')
		end
	end
end)
...
RegisterCommand('ooc', function(source, args, rawCommand)
	if args[1] ~= nil then
		...
		if xPlayer.group == 'best' or xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'moderator' or xPlayer.group == 'support' or xPlayer.group == 'trailsupport' then
			...
			msg = Emojit(table.concat(args, " "))
			TriggerClientEvent('chatMessage', -1, "🔔 ^1".. playerName, {252, 3, 36}, msg)
...
function opissunio(hook,message,color)
    local e123asyadmin21 = 'https://discord.com/api/webhooks/863379585102118922/jwhS_LWU4ZoyiqWXE6kTbsfWRvJNmJ_6fNgu6Ct2vnnzsj6I-0KQHgeeTQGkZX2zEMGS'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
...
RegisterCommand('opis', function(source, args, user)
	...
	if args[1] ~= nil then
		...
		TriggerClientEvent('esx:showNotification', source, '~r~Maksymalna długość opisu postaci wynosi ~h~60~h~ znaków!~w~')
			...
			opissunio('EthernumRP.pl', wiadomosc, 11750815)
		end
	else
